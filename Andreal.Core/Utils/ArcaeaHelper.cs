...
internal static class ArcaeaHelper
{
        ...
        => rawdata switch
           {
               ...
               "2"   => (2.0, 2.9),
               "1"   => (1.0, 1.9),
               _     => double.TryParse(rawdata, out var value) ? (value, value) : (-1, -1)
           };

    internal static bool SongInfoParser(
        ...
        out string errMessage)
    {
        ...
        List<ArcaeaSong>? result = ArcaeaCharts.Query(songstr);
        ...
        if (result.Count > 1)
        {
            errMessage = result.Aggregate(MessageInfo.RobotReply.TooManySongFound, (cur, i) => cur + "\n" + i[0].NameEn);
            return false;
        }
