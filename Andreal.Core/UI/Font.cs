...
internal static class Font
{
    static Font()
    {
        ...
        foreach (var s in new DirectoryInfo(Path.ArcaeaFontRoot).GetFiles().Select(s => s.FullName))
            pfc.AddFontFile(s);
        _fontFamily = pfc.Families.ToDictionary(i => i.Name, i => i);
    }
    ...
    internal static readonly System.Drawing.Font Exo64 = GetFont("Exo", 64),
